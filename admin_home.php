...
table tr:hover td{
	...
	$admin_query = mysql_query("SELECT * FROM `user-info` where uid = '$uid'") or die(mysql_error());
	$admin_row = mysql_fetch_array($admin_query) or die("Failed " . mysql_error());
	
	$name = $admin_row["firstname"] . " " . $admin_row["lastname"];
	
	$query = mysql_query("SELECT * FROM `user-info` where (usertype=\"doctor\" OR usertype=\"admin\") AND active=0") or die(mysql_error());
	...
	<br><br><br><br>
	<div id="logo"></div><br>
	<p>Welcome <?php print $name ?>!</p><form action="/wad/logout"><input type="submit" value="Logout"></form>
	<div id="approve"></div>
	<table cellspacing='0'>
